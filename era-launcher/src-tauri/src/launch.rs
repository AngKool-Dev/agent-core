...
impl LaunchEngine {
    ...
    pub async fn launch(
        ...
        self.emit_status(window, "PREPARING", "Preparing instance...".to_string())
            .await?;
        ...
        .await?;
        let client_jar = self
            .download_client(&version_info, &instance_dir, req.fresh, window)
            .await?;

        self.emit_status(window, "DOWNLOADING", "Resolving libraries...".to_string())
            .await?;
        let libs = self
            .download_libraries(&version_info, &instance_dir, req.fresh, window)
            .await?;

        self.emit_status(window, "VERIFYING", "Extracting natives...".to_string())
            .await?;
        ...
        self.emit_status(window, "DOWNLOADING", "Downloading assets...".to_string())
            .await?;
        self.download_assets(&version_info, &assets_dir, window)
            .await?;

        self.emit_status(window, "VERIFYING", "Verifying files...".to_string())
            .await?;
        ...
        std::thread::spawn(move || {
            if let Some(stdout) = stdout {
                ...
                loop {
                    ...
                    let _ = n;
                    if !line.is_empty() {
                        let _ = window_clone.emit("minecraft:stdout", line.trim_end().to_string());
                    }
                }
            }
            if let Some(stderr) = stderr {
                ...
                loop {
                    ...
                    let _ = n;
                    if !line.is_empty() {
                        let _ = window_clone.emit("minecraft:stderr", line.trim_end().to_string());
                    }
                }
            }
            let _ = window_clone.emit("minecraft:exit", pid);
        });
    ...
    async fn download_client(
        ...
        if let Some(dl) = info.downloads.as_ref().and_then(|d| d.client.as_ref()) {
            let dm = self.download_with_progress(window, &info.id);
        ...
        let cb: Arc<dyn Fn(crate::downloads::DownloadProgress) + Send + Sync> =
            Arc::new(move |progress| {
                let _ = window.emit(
                    "minecraft:download",
                    serde_json::json!({
                        "file": if progress.is_complete {
                            prefix.clone()
                        } else {
                            format!("{}_{}", prefix, progress.file_name)
                        },
    ...
    async fn download_assets(
        ...
        if !index_path.exists() {
            let dm = self.download_with_progress(window, &info.asset_index.id);
